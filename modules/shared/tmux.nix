# Shared tmux config for the NixOS hosts (framework-desktop, lenovo).
# Session restoration + screen-like keybindings + OSC 52 clipboard.
# darwin (apple-macbook-air-m2) keeps its own module: the nix-darwin
# programs.tmux surface differs enough that it is not worth a conditional.
{ config, pkgs, ... }:
