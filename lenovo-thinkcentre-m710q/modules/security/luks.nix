...

{
  ...
  boot.initrd = {
    ...
    # Use DHCP for reliability (check your DHCP server to reserve 192.168.1.42 for Congo)
    systemd.network = {
      enable = true;
    ...
    systemd.services.display-ip = {
      ...
      script = ''
        sleep 5
        IP=$(ip -4 addr show enp0s31f6 | sed -n 's/.*inet \([0-9.]*\).*/\1/p' | head -1)
        if [ -n "$IP" ]; then
          MSG="
          ========================================
          SSH available at: $IP:2222
          Tailscale: ssh -p 2222 root@congo-initrd
          Local: ssh -p 2222 root@$IP
          ========================================"
...
if [ -n "$SSH_CONNECTION" ] || [ -n "$SSH_CLIENT" ]; then
  ...
  /bin/systemd-cryptsetup attach crypted /dev/disk/by-uuid/806af537-855c-4e61-aa0a-87e0c08df2da
...
EOF
  ...
  boot.initrd.luks.devices = {
    "crypted" = {
      # Use the UUID of the encrypted partition (sda2)
      device = "/dev/disk/by-uuid/806af537-855c-4e61-aa0a-87e0c08df2da";
      preLVM = true;
