  ...
  environment.systemPackages = [
    (pkgs.writeScriptBin "darwin-auto-update" ''
      ...
      notify_failure() {
        ...
        # asuser runs osascript inside jason's GUI session. terminal-notifier
        # is dead on macOS 26 (it used NSUserNotification, removed in this
        # release), so it exits 0 and posts nothing. osascript's display
        ...
        /bin/launchctl asuser "$(/usr/bin/id -u jason)" /usr/bin/osascript \
          ...
          -e 'end run' \
          "darwin-auto-update failed" "See /var/log/darwin-auto-update.log" \
