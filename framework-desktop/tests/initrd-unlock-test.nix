...
then throw "initrd-unlock check failed for ${hostName}: ${failedNames}"
  ...
  ''
    ...
    # Best-effort: peek inside the produced initrd and confirm the tailscaled
    # binary actually made it in. systemd-initrd uses a cpio archive that may
    # be compressed with zstd, xz, or gzip. Try each.
    mkdir extracted
    cd extracted
